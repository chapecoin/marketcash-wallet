...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_marketcashDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfMarketCashBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 marketcashHeight;
  if (!checkIfBlockchainOutdated(currentHeight, marketcashHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(marketcashHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfMarketCashBlockchainExists() const {
  return m_marketcashDir.exists() && m_marketcashDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_marketcashDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::marketcashBlockchainInfo() const {
  return QFileInfo(m_marketcashDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_marketcashDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_marketcashDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
