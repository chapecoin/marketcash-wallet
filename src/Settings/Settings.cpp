...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("marketcashwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("marketcashwallet.desktop");
  ...
  res = autorunSettings.contains("MarketCashWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("MarketCashWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.marketcash.marketcash scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=MarketCash Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("MarketCashWallet", appPath);
    } else {
      autorunSettings.remove("MarketCashWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\marketcash", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:marketcash");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\marketcash\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\marketcash\\shell\\open\\command", QSettings::NativeFormat);
