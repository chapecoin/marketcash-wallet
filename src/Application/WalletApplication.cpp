...
const char BYTECOIN_URI_SCHEME_NAME[] = "marketcash";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("marketcashwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("marketcashwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] MarketCash wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "MarketCash wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have marketcashd with non-default RPC port, you should terminate it and relaunch MarketCashWallet\n"
      "or\n"
      "Set the Local deamon required port in MarketCashWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
